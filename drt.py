...
def calcula_abreviaturas (maxAbrev, textos, compatible):
  ...
  for i in range (num_abreviaturas):
    ...
    posMaxAhorro = None
    sconjto      = None
    if i < limiteSuper:  # En las últimas, es poco probable que se aproveche esto
...
parser.add_argument ('-s', '--superset-limit', metavar = _('SUPERSET_LIMIT'), type = int, help = _('limit for the superset search heuristic (default: %(default)d)'), default = 100)
...
limiteSuper = args.superset_limit
